...
COMPONENT_VERSION = Fb2kVersions.get("scrobble")
BUILD_NUMBER = Fb2kVersions.get_build_number("scrobble")
...
uuid_lastfm_group = generate_uuid
...
lastfm_files = Dir.glob("src/LastFm/*.{cpp,h,mm}").map { |f| File.basename(f) }
services_files = Dir.glob("src/Services/*.{cpp,h,mm}").map { |f| File.basename(f) }
...
[core_files, lastfm_files, services_files, ui_files, integration_files].flatten.each do |file|
...
puts "  LastFm files: #{lastfm_files.join(', ')}"
...
[['Core', core_files], ['LastFm', lastfm_files], ['Services', services_files], ['UI', ui_files], ['Integration', integration_files]].each do |group, files|
  files.each do |file|
