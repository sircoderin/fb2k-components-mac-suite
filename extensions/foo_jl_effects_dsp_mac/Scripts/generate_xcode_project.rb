#!/usr/bin/env ruby
# Generate Xcode project for foo_jl_effects_dsp_mac
...
PROJECT_NAME = "foo_jl_effects_dsp"
BUNDLE_ID = "com.foobar2000.foo-jl-effects-dsp"
SDK_PATH = Fb2kSdk.path

# Version from shared/version.h (single source of truth)
COMPONENT_VERSION = Fb2kVersions.get("effects_dsp")
BUILD_NUMBER = Fb2kVersions.get_build_number("effects_dsp")
...
uuid_effects_group = generate_uuid
uuid_ui_group = generate_uuid
uuid_integration_group = generate_uuid
uuid_thirdparty_group = generate_uuid
uuid_freeverb_group = generate_uuid
uuid_soundtouch_group = generate_uuid
...
effects_files = Dir.glob("src/Effects/*.{cpp,h,mm}").map { |f| File.basename(f) }
...
freeverb_files = Dir.glob("src/ThirdParty/Freeverb/*.{cpp,h}").map { |f| File.basename(f) }
soundtouch_files = Dir.glob("src/ThirdParty/SoundTouch/*.{cpp,h}").map { |f| File.basename(f) }
...
[core_files, effects_files, ui_files, integration_files, freeverb_files, soundtouch_files].flatten.each do |file|
  # Use prefixed keys for ThirdParty files to avoid name collisions
...
puts "  Effects files: #{effects_files.join(', ')}"
puts "  UI files: #{ui_files.join(', ')}"
puts "  Integration files: #{integration_files.join(', ')}"
puts "  Freeverb files: #{freeverb_files.join(', ')}"
puts "  SoundTouch files: #{soundtouch_files.join(', ')}"
...
[['Core', core_files], ['Effects', effects_files], ['UI', ui_files], ['Integration', integration_files], ['Freeverb', freeverb_files], ['SoundTouch', soundtouch_files]].each do |group, files|
  files.each do |file|
...
/* Begin PBXGroup section */
		...
		#{uuid_src_group} /* src */ = {
			...
			children = (
				...
				#{uuid_effects_group} /* Effects */,
				#{uuid_ui_group} /* UI */,
				#{uuid_integration_group} /* Integration */,
				#{uuid_thirdparty_group} /* ThirdParty */,
			);
			path = src;
			sourceTree = "<group>";
		};
...
pbxproj_content += <<~PBXPROJ
		...
		#{uuid_thirdparty_group} /* ThirdParty */ = {
			isa = PBXGroup;
			children = (
				#{uuid_freeverb_group} /* Freeverb */,
				#{uuid_soundtouch_group} /* SoundTouch */,
			);
			path = ThirdParty;
			sourceTree = "<group>";
		};
		...
		#{uuid_debug_config_target} /* Debug */ = {
			...
			buildSettings = {
				...
				HEADER_SEARCH_PATHS = (
					...
					"$(PROJECT_DIR)/src",
					"$(PROJECT_DIR)/src/ThirdParty/SoundTouch",
				);
